...
// Import calculation functions from portfolio calculations
import { calculateReturns, calculateEWMAVolatility, calculatePearsonCorrelation } from './portfolioCalculations.js';
 ...
 */
const calculateRollingVolatility = (prices, windowSize = 60, lambda = 0.94) => {
 ...
 */
const calculateMovingAverages = (prices, shortWindow = 21, longWindow = 63) => {
 ...
 */
const calculateRollingCorrelations = (normalizedData, windowSize = 60) => {
 ...
 */
const classifyVolatilityRegime = (volatility, thresholds) => {
 ...
 */
const classifyTrendRegime = (signal, threshold) => {
  if (signal > threshold) {
 ...
 */
const classifyCrisisRegime = (correlation, volatility, volatilityHistory, thresholds) => {
 ...
 */
export const detectMarketRegimes = (normalizedData, params = DEFAULT_REGIME_PARAMS) => {
 ...
 * @param {Object} currentRegime - Current regime classification
 * @param {Object} baseParams - Base parameters to adjust
 * @returns {Object} Adjusted parameters for portfolio strategies
 */
export const getAdaptiveParameters = (currentRegime, baseParams = null) => {
  ...
  switch (currentRegime.volatilityRegime) {
    case REGIME_TYPES.VOLATILITY.LOW:
      adjusted.ewmaLambda = Math.max(0.85, base.ewmaLambda - 0.05);  // More responsive
      adjusted.rebalanceFreqDays = Math.min(42, Math.floor(base.rebalanceFreqDays * 1.3)); // Less frequent
      break;
    case REGIME_TYPES.VOLATILITY.HIGH:
      adjusted.ewmaLambda = Math.min(0.98, base.ewmaLambda + 0.03);  // Less responsive
      adjusted.rebalanceFreqDays = Math.max(10, Math.floor(base.rebalanceFreqDays * 0.7)); // More frequent
      break;
    // MEDIUM volatility keeps base parameters
  }

  // Adjust based on trend regime
  switch (currentRegime.trendRegime) {
    case REGIME_TYPES.TREND.BULL:
      adjusted.leverage = Math.min(2.5, base.leverage * 1.1);  // Slightly higher leverage
      break;
    case REGIME_TYPES.TREND.BEAR:
      adjusted.leverage = Math.max(1.2, base.leverage * 0.8);  // Lower leverage
      adjusted.correlationPenalty = Math.min(1.0, base.correlationPenalty * 1.2); // Higher correlation penalty
      break;
    // SIDEWAYS keeps base parameters
  }

  // Adjust based on crisis regime (overrides other adjustments for safety)
  switch (currentRegime.crisisRegime) {
    case REGIME_TYPES.CRISIS.CRISIS:
      adjusted.ewmaLambda = 0.98;  // Maximum smoothing
      adjusted.rebalanceFreqDays = 5;   // Almost daily rebalancing
      adjusted.leverage = 1.2;     // Minimal leverage
      adjusted.correlationPenalty = 1.0; // Maximum correlation penalty
      break;
    case REGIME_TYPES.CRISIS.STRESS:
      ...
      break;
    // NORMAL keeps other regime adjustments
  }
  ...
  adjusted.leverage = Math.max(1.0, Math.min(3.0, adjusted.leverage));
 ...
 */
export const getCurrentRegime = (regimes, targetDate = null) => {
 ...
 */
export const validateRegimeDetection = (regimes) => {
  if (!regimes || regimes.length === 0) {
...
// Export default parameters for external use
export const DEFAULT_PARAMETERS = DEFAULT_REGIME_PARAMS;